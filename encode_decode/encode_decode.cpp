...
{
    static std::array<std::uint8_t, 256> conventional_to_dual_basis = {
    ...
    static std::array<std::uint8_t, 256> dual_basis_to_conventional = {
    ...
    static RS::ReedSolomon<block_size, parity_symbols> rs;
    ...
    void Encode(std::span<std::uint8_t> src, std::vector<std::uint8_t> &dst)
    {
        ...
        for (int i = 0; i < total_blocks; i++)
        {
            ...
            ConvertBases(block, true, block_size);
            rs.Encode(block, encoded.data() + i * rs_length);
        }

        ConvertBases(encoded.data(), false, rs_length * total_blocks);
    ...
    void ConvertBases(uint8_t *data, bool dual_to_alpha, size_t size)
    {
        if (!dual_to_alpha)
        {
            ...
            {
                data[i] = conventional_to_dual_basis[data[i]];
            }
        }
        else
        {
            for (int i = 0; i < size; i++)
            {
                data[i] = dual_basis_to_conventional[data[i]];
            }
        }
    }
    ...
    void Decode(std::span<std::uint8_t> src, std::vector<std::uint8_t> &dst)
    {
        ...
        for (int i = 0; i < total_blocks; i++)
        {
            uint8_t *current_block = buffer.data() + i * rs_length;
            uint8_t decoded_block[block_size] = {0};
            ConvertBases(current_block, true, rs_length);
            rs.Decode(current_block, decoded_block);
            ConvertBases(decoded_block, false, block_size);
            int start_idx = i * block_size;
