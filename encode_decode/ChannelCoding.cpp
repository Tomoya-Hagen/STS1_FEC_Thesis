#include "ChannelCoding.hpp"
#include "ReedSolomon.hpp"

#include <algorithm>
#include <array>
#include <cstddef>     // for Byte and std::to_integer
#include <cstdint>
#include <iostream>
#include <span>
#include <vector>

namespace sts1cobcsw
{
    // Polynomial: 0b01011111
    constexpr auto unscrambleLookUpTable = std::to_array<std::uint8_t>({
        });
    // Polynomial: 0b10101001
    constexpr auto scrambleLookUpTable = std::to_array<std::uint8_t>({
        });
    static_assert(unscrambleLookUpTable.size() == 255);
    static_assert(scrambleLookUpTable.size() == 255);


    auto Unscramble(std::span<Byte> data) -> void;
    auto Scramble(std::span<Byte> data) -> void;


    auto Encode(std::span<Byte, blockLength> data) -> void
    {
        RSEncode(static_cast<data_t*>(data.data()));
        Scramble(data);
    }


    auto Decode(std::span<Byte, blockLength> data) -> void
    {
        Unscramble(data);
        RSDecode(static_cast<data_t*>(data.data()));
    }


    auto Unscramble(std::span<Byte> data) -> void
    {
        for (int i = 0; i < data.size(); i++)
        {
            data[i] ^= static_cast<Byte>(unscrambleLookUpTable[i % 255]);
        }
    }


    auto Scramble(std::span<Byte> data) -> void
    {
        for (int i = 0; i < data.size(); i++)
        {
            data[i] ^= static_cast<Byte>(scrambleLookUpTable[i % 255]);
        }
    }
}
